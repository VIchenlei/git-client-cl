...
export default class CardStore {
  ...
  addFields(cardID, card) {
    ...
    if (cardTypeID === 1 && xdata.objRange === 1) {
      // 只看正式员工
      let staffExtends = xdata.metaStore.data.staff_extend
      let obj_id = card[CARD.obj_id]
      let extend = staffExtends && staffExtends.get(obj_id)
      let deptIDCk = extend && extend.dept_id
      deptIDCk && card.splice(12, 1, deptIDCk)
    }
  ...
  processStaffNumber(stat, cards) {
    ...
    const areas = xdata.metaStore.data.area
    statNames.forEach(name => {
      let datas = stat[name]
      datas.forEach(data => {
        let rows = cards.filter(item => item[CARD[`${name}_id`]] === data[0])
        data[1] = rows ? rows.length : data[1]
  ...
  judgeCardState (cardID, card, needMove, type) {
    if (type === 'vehicle' || card[CARD.speed] > 0) {
      ...
      // 之前有没有卡||切换地图||上下猴车||状态是否静止||是否进入盲区
      if (!prescard || this.changeMap || (ispreMonkey != isMonkey && (ispreMonkey == 7 || isMonkey == 7)) || (ispreMonkey !== isMonkey && (ispreMonkey == 0 || isMonkey == 0)) || (ispreUncover != isUncover && (ispreUncover == 1024 || isUncover == 1024))) {
        this.showCard(cardID, card, needMove)
      }
    }
  ...
  processDetail(data, needMove, type) { // type暂时增加
    ...
    if (data) {
      for (let i = 0, len = data.length; i < len; i++) {
        ...
        let condition = xdata.metaStore.needDisplay(cardID) && (!xdata.depts || xdata.depts.includes(deptID))
        if (condition) {
          if (xdata.roleID != 1 && areaNeedDisplay === 0) {
